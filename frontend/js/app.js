...
/* ─────────────────────────────────────────────
   ADMIN CONFIG — change this to your admin email
───────────────────────────────────────────── */
const ADMIN_EMAIL = 'admin@trij.com';
...
const state = {
  ...
  toastTimer: null,
  adminTab: 'users',
  adminData: { users: [], bookings: [], receipts: [] },
};
...
function refreshPortalView() {
  if (state.user) {
    ...
    document.getElementById('portalDashboard').style.display = isAdmin ? 'none' : '';
    document.getElementById('adminDashboard').style.display = isAdmin ? '' : 'none';
    if (isAdmin) {
      document.getElementById('adminWelcome').textContent = `Admin Panel`;
...
async function loadAdminData() {
  try {
    const [usersRes, bookingsRes, receiptsRes] = await Promise.all([
    ...
    state.adminData.users    = usersData.users    || [];
    state.adminData.bookings = bookingsData.bookings || [];
    state.adminData.receipts = receiptsData.receipts || [];
...
function renderAdminStats() {
  ...
  if (el('adminUserCount'))    el('adminUserCount').textContent    = state.adminData.users.length;
  if (el('adminBookingCount')) el('adminBookingCount').textContent = state.adminData.bookings.length;
  const total = state.adminData.receipts.reduce((s, r) => s + (Number(r.amount) || 0), 0);
  if (el('adminRevenue'))      el('adminRevenue').textContent      = `$${total}`;
}
...
function renderAdminUsers() {
  ...
  if (state.adminData.users.length === 0) {
    list.innerHTML = `<div class="empty-state"><span class="es-icon">👤</span>No users yet.</div>`;
    return;
  }
  list.innerHTML = state.adminData.users.map(u => `
    <div class="admin-row">
      <div class="admin-row-info">
        <div class="admin-row-name">${u.full_name || u.name || '—'}</div>
        <div class="admin-row-sub">${u.email} · ${u.role || 'user'}</div>
      </div>
      <button class="admin-del-btn" onclick="adminDeleteUser(${u.id}, '${(u.full_name||u.name||'').replace(/'/g,"\\'")}')">🗑 Delete</button>
    </div>`).join('');
}
...
function renderAdminBookings() {
  ...
  if (!list) return;
  if (state.adminData.bookings.length === 0) {
    list.innerHTML = `<div class="empty-state"><span class="es-icon">📅</span>No bookings yet.</div>`;
    return;
  }
  list.innerHTML = state.adminData.bookings.map(b => `
    <div class="admin-row">
      <div class="admin-row-info">
        <div class="admin-row-name">${b.sport || '—'} · ${b.coach || '—'}</div>
        <div class="admin-row-sub">${b.user_email} · ${b.date || '—'} ${b.time || ''} · ${b.package_name || '—'}</div>
      </div>
      <button class="admin-del-btn" onclick="adminDeleteBooking(${b.id})">🗑 Delete</button>
    </div>`).join('');
}

function renderAdminReceipts() {
  const list = document.getElementById('adminReceiptsList');
  if (!list) return;
  if (state.adminData.receipts.length === 0) {
    list.innerHTML = `<div class="empty-state"><span class="es-icon">🧾</span>No payments yet.</div>`;
    return;
  }
  list.innerHTML = state.adminData.receipts.map(r => `
    <div class="admin-row">
      <div class="admin-row-info">
        <div class="admin-row-name">$${r.amount} · ${r.sport || '—'} · ${r.package_name || '—'}</div>
        <div class="admin-row-sub">${r.user_email} · Coach: ${r.coach || '—'} · ••••${r.card_last4 || '????'} · ${r.date || '—'}</div>
      </div>
      <span class="receipt-status">✅ Paid</span>
    </div>`).join('');
}
